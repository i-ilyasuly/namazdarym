...
const radialData = [{ name: "Орындалды", value: 80 }];
...
function CustomRadarTick({ payload, x, y, cx, cy, ...rest }: any) {
  ...
  switch (payload.value) {
    case "Таң":
      IconToRender = EclipseIcon;
      break;
    case "Бесін":
      IconToRender = SunIcon;
      break;
    case "Екінті":
      IconToRender = SunDimIcon;
      break;
    case "Шам":
      IconToRender = SunsetIcon;
      break;
    case "Құптан":
      IconToRender = MoonStarIcon;
      break;
    default:
      return null;
  }
  ...
  return (
    <g transform={`translate(${posX}, ${posY})`}>
      <IconToRender animated={true} size={iconSize} color={iconColor} />
    </g>
  );
}
...
function PieChartWithPaddingAngle({
  ...
  if (Platform.OS !== "web") {
    return (
      <Text style={{ fontFamily, color: isDark ? "#a1a1aa" : "#1c1c1e" }}>
        Диаграмма тек браузерде жұмыс істейді
      </Text>
    );
  }
  return (
...
function PrayerRadarChart({
  ...
  if (Platform.OS !== "web") {
    return (
      <Text style={{ fontFamily, color: isDark ? "#a1a1aa" : "#1c1c1e" }}>
        Радар тек браузерде жұмыс істейді
      </Text>
    );
  }
  return (
    <View style={{ width: "100%", height: 220, alignItems: "center" }}>
      <ResponsiveContainer width="100%" height="100%">
        <RadarChart cx="50%" cy="50%" outerRadius="65%" data={finalData}>
          ...
          <PolarAngleAxis dataKey="name" tick={<CustomRadarTick />} />
          <PolarRadiusAxis
            angle={30}
...
function PrayerLineChart({
  ...
  if (Platform.OS !== "web") {
    return (
      <Text style={{ fontFamily, color: isDark ? "#a1a1aa" : "#1c1c1e" }}>
        График тек браузерде жұмыс істейді
      </Text>
    );
  }
  return (
...
function PrayerAreaChart({
  ...
  if (Platform.OS !== "web") {
    return (
      <Text style={{ fontFamily, color: isDark ? "#a1a1aa" : "#1c1c1e" }}>
        Аудандық диаграмма тек браузерде жұмыс істейді
      </Text>
    );
  }
  return (
...
function PrayerBarChart({
  ...
  if (Platform.OS !== "web") {
    return (
      <Text style={{ fontFamily, color: isDark ? "#a1a1aa" : "#1c1c1e" }}>
        Бағаналық диаграмма тек браузерде жұмыс істейді
      </Text>
    );
  }
  return (
...
function MonthlyCalendar({ records }: { records: any[] }) {
  const { isDark, themeColors, colorMode } = useAppTheme();
  ...
  const getDayDotColor = (day: number) => {
    ...
    const statuses = [record.fajr, record.dhuhr, record.asr, record.maghrib, record.isha];
    const score = (s: string) => {
      if(s === 'jamaat') return 4;
      if(s === 'on_time') return 3;
      if(s === 'late') return 2;
      if(s === 'qaza') return 1;
      return 0;
    }
    const sum = statuses.reduce((a, b) => a + score(b), 0);
    if(sum === 0) return null; // No data
    
    const maxStatus = statuses.reduce((a, b) => score(a) > score(b) ? a : b);
    if(maxStatus === 'jamaat') return statusColors.jamaat;
    if(maxStatus === 'on_time') return statusColors.on_time;
    if(maxStatus === 'late') return statusColors.late;
  ...
  return (
    ...
    >
      <View style={styles.calendarHeader}>
        ...
        <Text
          style={[styles.calendarTitle, { color: isDark ? "#fff" : "#1c1c1e" }]}
        >
      ...
      <View style={styles.calendarGrid}>
        {dayLabels.map((label) => (
          <Text
            key={label}
            style={[styles.dayLabel, { color: isDark ? "#a1a1aa" : "#8e8e93" }]}
          >
            {label}
          </Text>
        ))}
            ...
            {day !== null && (
              <View
                style={[
                  styles.dayTouch,
                  isToday(day) && {
                    backgroundColor: isDark ? '#3a3a3c' : '#f1f1f6',
                    borderRadius: 10,
                  },
                ]}
              >
                <Text
                  style={[
                    styles.dayText,
                    { color: isDark ? "#fff" : "#1c1c1e" },
                    isToday(day) && { fontWeight: "800", color: themeColors[0] },
                  ]}
                >
                  {day}
                </Text>
                ...
                <View style={styles.dotsRow}>
                  {dotColor && <View style={[styles.miniDot, { backgroundColor: dotColor, transform: [{scale: 1.2}] }]} />}
                </View>
              </View>
            )}
          </View>
...
export default function StatsScreen() {
  const { user } = useAuth();
  const [activeSegment, setActiveSegment] = useState<"stats" | "calendar" | "future_stats">(
    "stats",
  );
  
  // States for new Stats Tab
  const [statsPeriod, setStatsPeriod] = useState<number>(30);
  const [statsStatus, setStatsStatus] = useState<string>("all");
  const [activeNewChart, setActiveNewChart] = useState<string>("donut");
  const [activeChart, setActiveChart] = useState<
    "pie" | "radar" | "line" | "area" | "bar"
  >("pie");
  ...
  const handleGenerateMock = async () => {
    ...
    try {
      await updateDoc(doc(db, "users", user.uid), {
        ...
        history: Array.from(
          { length: 7 },
          (_, i) => Math.floor(Math.random() * 50) + 50 + i * 10,
        ),
      });
    } catch (err) {
  ...
  // new charts data processor
  const getNewStatsData = () => {
    const periodRecords = mockRecords.slice(0, statsPeriod);
    const prayers = ["fajr", "dhuhr", "asr", "maghrib", "isha"];
    
    return prayers.map(prayerKey => {
      let prayed = 0;
      let missed = 0;
      let congregation = 0;
      let delayed = 0;
      ...
      periodRecords.forEach(r => {
        const val = r[prayerKey];
        if (statsStatus !== "all" && val !== statsStatus) return;

        if (val === "on_time") prayed++;
        else if (val === "jamaat") { prayed++; congregation++; }
        else if (val === "late") { prayed++; delayed++; }
        else if (val === "qaza") missed++;
      });
  ...
  const displayRecords =
    mockRecords.length > 0
      ? mockRecords.slice(0, getDaysForPeriod(activePeriod))
      : [];
    ...
    displayRecords.length > 0
      ? (() => {
          ...
          displayRecords.forEach((r) => {
            (["fajr", "dhuhr", "asr", "maghrib", "isha"] as const).forEach(
              (p) => {
          ...
          return [
            {
              name: statusLabels.jamaat,
              value: counts.jamaat,
              fill: statusColors.jamaat,
            },
            {
              name: statusLabels.on_time,
              value: counts.on_time,
              fill: statusColors.on_time,
            },
            {
            ...
            {
              name: statusLabels.qaza,
              value: counts.qaza,
              fill: statusColors.qaza,
            },
          ];
        })()
      : [
          ...
          .map((r) => ({
            day:
              displayRecords.length > 7
                ...
                : r.day,
            value:
              [r.fajr, r.dhuhr, r.asr, r.maghrib, r.isha].filter(
                (s) => s === "on_time" || s === "jamaat",
              ).length * 20,
          }))
      : lineData;
  ...
  if (currentLineData.length > 15) {
    ...
    currentLineData = currentLineData.filter(
      (_, i) => i % step === 0 || i === currentLineData.length - 1,
    );
  }
    ...
    displayRecords.length > 0
      ? (() => {
          ...
          return [
            {
              name: "Таң",
              value: Math.round((counts.fajr / total) * 100) || 0,
            },
            {
              name: "Бесін",
              value: Math.round((counts.dhuhr / total) * 100) || 0,
            },
            {
              name: "Екінті",
              value: Math.round((counts.asr / total) * 100) || 0,
            },
            {
              name: "Шам",
              value: Math.round((counts.maghrib / total) * 100) || 0,
            },
            {
              name: "Құптан",
              value: Math.round((counts.isha / total) * 100) || 0,
            },
          ];
        })()
      : [
  ...
  return (
    ...
    >
      ...
      <View style={styles.header}>
        <Text
          style={[styles.headerTitle, { color: isDark ? "#fff" : "#1c1c1e" }]}
        >
          Зертхана
        </Text>
      </View>
      ...
      <View style={styles.segmentContainer}>
        ...
        >
          <TouchableOpacity
            style={[
              styles.segmentBtn,
              activeSegment === "stats" && styles.segmentBtnActive,
              activeSegment === "stats" &&
          ...
          >
            ...
            <Text
              style={[
                styles.segmentText,
                activeSegment === "stats" && styles.segmentTextActive,
                activeSegment === "stats" && isDark && { color: "#fff" },
              ]}
            >
              Зертхана
            </Text>
          ...
          <TouchableOpacity
            style={[
              styles.segmentBtn,
              activeSegment === "calendar" && styles.segmentBtnActive,
          ...
          >
            ...
            <Text
              style={[
                ...
                activeSegment === "calendar" && isDark && { color: "#fff" },
              ]}
            >
              Күнтізбе
            </Text>
          ...
          <TouchableOpacity
            style={[
              styles.segmentBtn,
              activeSegment === "future_stats" && styles.segmentBtnActive,
              activeSegment === "future_stats" &&
                isDark && { backgroundColor: "#3a3a3c" },
            ]}
            activeOpacity={0.8}
            onPress={() => setActiveSegment("future_stats" as any)}
          >
            <LineChartIcon
              color={
                activeSegment === "future_stats"
            ...
            <Text
              style={[
                styles.segmentText,
                activeSegment === "future_stats" && styles.segmentTextActive,
                activeSegment === "future_stats" && isDark && { color: "#fff" },
              ]}
            >
              Статистика
            </Text>
                ...
                {[
                  ...
                  >
                    ...
                    <Text
                      style={[
                        styles.periodText,
                        activeChart === c.id && styles.periodTextActive,
                        activeChart === c.id && isDark && { color: "#fff" },
                      ]}
                    >
                      {c.title}
                    </Text>
              ...
              >
                {PERIODS.map((p) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.periodBtn,
                      activePeriod === p.id && styles.periodBtnActive,
                      activePeriod === p.id &&
                        isDark && { backgroundColor: "#3a3a3c" },
                    ]}
                  ...
                  >
                    <Text
                      style={[
                        styles.periodText,
                        activePeriod === p.id && styles.periodTextActive,
                        activePeriod === p.id && isDark && { color: "#fff" },
                      ]}
                    >
                      {p.title}
                    </Text>
        ...
        {activeSegment === "stats" ? (
          <View style={styles.statsContainer}>
            ...
            >
              ...
              <View style={{ flex: 1, minHeight: 400, marginTop: 16 }}>
                {Platform.OS === 'web' ? (
                  <View style={{ paddingHorizontal: 16, height: 400 }}>
            ...
            <View style={styles.metricContainer}>
              {[
                {
                  title: "Үздіксіз",
                  value: displayRecords.length > 0 ? "45 күн" : "12 күн",
                  icon: Flame,
                  color: themeColors[2],
                },
                {
                  ...
                  value:
                    displayRecords.length > 0
                      ? `${Math.round((currentChartData.find((d: any) => d.name === statusLabels.on_time)?.value / Math.max(1, displayRecords.length * 5)) * 100 || 0)}%`
                      : "85%",
                  icon: Target,
                  color: themeColors[0],
                },
                {
                  ...
                  value:
                    displayRecords.length > 0
                      ? `${Math.round((currentChartData.find((d: any) => d.name === statusLabels.jamaat)?.value / Math.max(1, displayRecords.length * 5)) * 100 || 0)}%`
                      : "4/5",
                  icon: Trophy,
                  color: themeColors[4] || "#fbbf24",
                },
              ].map((item, idx) => (
                <View
                  key={idx}
            ...
            >
              <View style={styles.heatmapHeader}>
                ...
                <Text
                  style={[
                    styles.chartTitle,
                    { color: isDark ? "#fff" : "#1c1c1e", marginBottom: 0 },
                  ]}
                >
                  Белсенділік картасы
                </Text>
              ...
              >
                <View style={{ flexDirection: "row", gap: 3 }}>
                  <View style={{ flexDirection: "column", gap: 3, marginRight: 2, justifyContent: 'space-between', paddingVertical: 1 }}>
                    <Text style={{ fontSize: 9, color: isDark ? '#a1a1aa' : '#8e8e93', fontFamily }}>Дс</Text>
                    <Text style={{ fontSize: 9, color: isDark ? '#a1a1aa' : '#8e8e93', fontFamily }}>Ср</Text>
                    <Text style={{ fontSize: 9, color: isDark ? '#a1a1aa' : '#8e8e93', fontFamily }}>Жм</Text>
                  </View>
                  {(() => {
                    const columns = [];
                    // Ensure we fill properly so we align visually with 7 days per column
                    for (let i = 0; i < currentHeatmapData.length; i += 7) {
                      const col = currentHeatmapData.slice(i, i + 7);
                      // Pad the last column if it's incomplete
                            ...
                            <View
                              ...
                              style={[
                                styles.heatmapCell,
                                {
                                  ...
                                  height: 11,
                                  borderRadius: 2,
                                  backgroundColor: val === -1 ? 'transparent' : (val > 0 ? themeColors[0] : (isDark ? "#27272a" : "#f4f4f5")),
                                  opacity: val === -1 ? 0 : (val === 0 ? 1 : (val === 1 ? 0.3 : val === 2 ? 0.5 : val === 3 ? 0.7 : 1))
                                },
                              ]}
                            />
                          ))}
          ...
          <MonthlyCalendar records={mockRecords} />
        ) : activeSegment === "future_stats" ? (
          <View style={styles.emptyState}>
            ...
            >
              <LineChartIcon color={themeColors[0]} size={24} />
            </View>
            <Text
              style={[
                styles.emptyTitle,
            ...
            <Text style={styles.emptyDesc}>
              Бұл бөлімде алдағы уақытта тереңдетілген статистика мен аналитика пайда болады.
            </Text>
          </View>
        ) : null}
      </ScrollView>
